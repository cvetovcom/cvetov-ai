...
function extractParams(message: string): Partial<ChatParams> {
  ...
  // 3. Поиск города - ищем слова с большой буквы
  const potentialCities = message.match(/[А-ЯЁ][а-яё]+(?:-[А-Яа-яЁё]+)?/g) || []

  for (const potentialCity of potentialCities) {
    // Простая проверка - если это не начало предложения и слово с большой буквы
    // то вероятно это город
    if (potentialCity.length > 2 && !RECIPIENTS.some(r => r.label === potentialCity) && !OCCASIONS.some(o => o.label === potentialCity)) {
      result.city = {
        name: potentialCity,
        slug: potentialCity.toLowerCase().replace(/\s+/g, '-')
      }
      break
    }
  }
